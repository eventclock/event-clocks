...
const TIME_ZONE_SUBDIVISIONS: Record<string, string> = {
...
export function canonicalizeTimeZone(timeZone: string) {
...
export function getTimeZoneMetadata(timeZone: string): TimeZoneMetadata {
  ...
  const countryCode = TIME_ZONE_COUNTRY_CODES[canonicalTimeZone];
  const countryName = countryCode
    ? countryNameFormatter?.of(countryCode) ?? countryCode
    : undefined;
  const subdivisionName = TIME_ZONE_SUBDIVISIONS[canonicalTimeZone];
  const cityName = titleizeSegment(canonicalTimeZone.split("/").at(-1) ?? canonicalTimeZone);
  const displayParts = [cityName, subdivisionName, countryName].filter(Boolean);
  ...
  return {
    ...
    subdivisionName,
    cityName,
    displayName: displayParts.length > 0 ? displayParts.join(", ") : cityName,
  };
}
