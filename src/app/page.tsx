    ...
    <main className="mx-auto max-w-5xl px-6 py-10">
      ...
      <header className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <nav className="flex flex-wrap gap-2">
          <a className="rounded-xl border border-violet-200/70 bg-white/60 px-3 py-2 text-sm font-semibold text-black/80 shadow-sm backdrop-blur hover:bg-white/80 dark:border-violet-300/20 dark:bg-white/5 dark:text-white/80 dark:hover:bg-white/10"
             href="/timezone">
            Timezone Converter
          </a>
          <a className="rounded-xl border border-black/10 bg-white/60 px-3 py-2 text-sm font-semibold text-black/70 shadow-sm backdrop-blur hover:bg-white/80 dark:border-white/10 dark:bg-white/5 dark:text-white/70 dark:hover:bg-white/10"
             href="/about">
            About
          </a>
          <a className="rounded-xl border border-black/10 bg-white/60 px-3 py-2 text-sm font-semibold text-black/70 shadow-sm backdrop-blur hover:bg-white/80 dark:border-white/10 dark:bg-white/5 dark:text-white/70 dark:hover:bg-white/10"
      ...
      <section className="grid gap-4 md:grid-cols-2">
        ...
        {/* Roadmap / “coming next” */}
        <div className="rounded-2xl border-2 border-violet-200/70 bg-white/60 p-6 shadow-sm backdrop-blur dark:border-violet-300/20 dark:bg-white/5">
          <div className="text-xl font-extrabold tracking-tight">Coming Next</div>
          <p className="mt-2 text-sm text-black/60 dark:text-white/60">
            A few more tools are planned. The goal is a small, high-quality set of utilities
            that stay fast and simple.
          </p>

          <ul className="mt-4 space-y-2 text-sm text-black/75 dark:text-white/75">
            <li className="flex gap-2">
              <span className="mt-1 inline-block h-2 w-2 rounded-full bg-violet-300 dark:bg-violet-400" />
              Meeting planner (find overlapping hours)
            </li>
            <li className="flex gap-2">
      ...
      {/* Small footer note area (optional) */}
      <section className="mt-8 rounded-2xl border border-black/10 bg-white/50 p-5 text-sm text-black/60 backdrop-blur dark:border-white/10 dark:bg-white/5 dark:text-white/60">
        Tip: Start with the Timezone Converter. Save favorites so you can compare with one click.
      </section>
    </main>
  );
}
